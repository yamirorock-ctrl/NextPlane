...
const CritiqueModal = ({ critique, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="bg-slate-900 border border-slate-700 rounded-3xl p-6 max-w-lg w-full shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 mb-6">
            <div className={`w-20 h-20 rounded-full flex items-center justify-center border-4 font-black text-3xl bg-slate-950 shadow-inner ${getScoreColor(critique.score)}`}>
                {critique.score}
            </div>
            <div>
                <h3 className="text-2xl font-bold text-white">Análisis Visual</h3>
                <p className="text-slate-400 text-sm flex items-center gap-1"><Sparkles size={12} className="text-indigo-400"/> Gemini Vision Pro</p>
            </div>
        </div>

        <div className="grid grid-cols-1 gap-3 mb-6">
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                <h4 className="font-bold text-white text-xs uppercase tracking-wider mb-1 flex items-center gap-2 opacity-80">
                    <Sun size={14} className="text-amber-400"/> Iluminación
                </h4>
                <p className="text-slate-300 text-sm leading-relaxed">{critique.critique?.lighting || "Sin datos"}</p>
            </div>
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                <h4 className="font-bold text-white text-xs uppercase tracking-wider mb-1 flex items-center gap-2 opacity-80">
                    <Maximize size={14} className="text-blue-400"/> Composición
                </h4>
                <p className="text-slate-300 text-sm leading-relaxed">{critique.critique?.composition || "Sin datos"}</p>
            </div>
             <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                <h4 className="font-bold text-white text-xs uppercase tracking-wider mb-1 flex items-center gap-2 opacity-80">
                    <Heart size={14} className="text-pink-400"/> Atractivo de Venta
                </h4>
                <p className="text-slate-300 text-sm leading-relaxed">{critique.critique?.appeal || "Sin datos"}</p>
            </div>
        </div>
        ...
        <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-xl relative overflow-hidden">
            ...
            <h4 className="font-bold text-emerald-400 text-sm mb-2 flex items-center gap-2">
                <Zap size={16} /> TIP DE MEJORA RÁPIDA
            </h4>
            <p className="text-emerald-100/90 text-sm font-medium relative z-10">
                "{critique.improvement_tip}"
            </p>
        </div>

        <button 
           onClick={onClose}
           className="w-full mt-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-indigo-900/40"
        >
           Entendido, ¡gracias!
        </button>
      </div>
